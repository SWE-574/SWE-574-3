...
def sync_schedule_details_from_scheduled_time(service):
    """#503: for non-group One-Time services where both fields exist, the
    human schedule_details copy must describe the same date/time as the
    ...
    Recurrent services are intentionally excluded — for them
    schedule_details carries the cadence ("Every Wednesday 19:00") while
    scheduled_time is just the next-occurrence anchor.
    """
