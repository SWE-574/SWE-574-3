...
test('NFR-03b: performing an admin action appends a new entry to the audit log', async ({ page }) => {
  ...
  // Wait for the initial log to render and capture the current row count.
  await page.waitForTimeout(2_000)

  const rowsBefore = await page
    .locator('table tr:not(:first-child), [role="row"]:not(:first-child)')
    .count()
...
test('NFR-03b: existing audit log entries are retained after a new action is appended', async ({ page }) => {
  ...
  // Capture text of the first existing log entry so we can verify retention.
  const firstEntryLocator = page
    .locator('table tr:not(:first-child), [role="row"]:not(:first-child)')
    .first()

  const hasExistingEntry = await firstEntryLocator.isVisible().catch(() => false)
  if (!hasExistingEntry) {
  ...
  const firstEntryText = await firstEntryLocator.innerText()
  ...
  // The previously existing entry must still be present (retention policy).
  await expect(page.getByText(firstEntryText.trim().slice(0, 40))).toBeVisible({ timeout: 10_000 })
})
