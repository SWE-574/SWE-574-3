...
function chipForSignals(signals?: ForYouSignals | null): SignalChip {
  if (!signals) return { label: 'For you', bg: 'rgba(168, 85, 247, 0.95)', fg: 'white' }
  const entries: Array<[string, number]> = [
    ['tag', signals.tag],
    ['follow', signals.follow],
    ['cooccur', signals.cooccur],
  ]
  ...
  if (topValue <= 0) {
    return { label: 'For you', bg: 'rgba(168, 85, 247, 0.95)', fg: 'white' }
  }
  if (topName === 'tag') {
    return { label: 'Matches your interests', bg: 'rgba(168, 85, 247, 0.95)', fg: 'white' }
  }
  if (topName === 'follow') {
    return { label: 'From your network', bg: 'rgba(245, 158, 11, 0.95)', fg: 'white' }
  }
  return { label: 'Popular with people like you', bg: 'rgba(59, 130, 246, 0.95)', fg: 'white' }
}
